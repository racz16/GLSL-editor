...
public class GlslCompletionProvider implements CompletionProvider {
    ...
    private void initializeCodeCompletion(CompletionResultSet completionResultSet, Document document, int caretOffset) throws BadLocationException {
    ...
    private void addCodeCompletionItems() throws BadLocationException, ParseException {
        ...
        Scope caretScope = GlslProcessor.getCaretScope(caretOffset);
    ...
    private void addUserFunctions(Scope scope) {
        for (Function function : scope.getFunctionPrototypes()) {
            if (function.getSignatureStopIndex() < caretOffset) {
                addElement(function);
            }
        }
    }
    ...
    private void addElementListIfDeclared(List<? extends Element> elements) {
        for (Element element : elements) {
            if (element.getStopIndex() < caretOffset) {
                addElement(element);
            }
        }
    }
    ...
    private void addElement(Element element) {
        if (element.getName().startsWith(filter)) {
            resultSet.addItem(new GlslCompletionItem((CompletionElement) element, filter.length(), caretOffset));
        }
    }
    ...
    private String computeFilter() throws BadLocationException {
        ...
        int offset = computeFilterOffset(text, caretOffset - 1);
        return offset == caretOffset - 1 ? "" : text.substring(offset + 1, caretOffset);
    }
